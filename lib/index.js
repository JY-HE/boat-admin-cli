...
import { checkPureOptions, hasTemplate } from './check.js';
...
// #region ------------------------- init 快速创建项目 -------------------------
cli.command('init <template-name> <project-name>', '快速创建一个新项目')
    .option('-f, --force', '如果目标文件存在，则强制覆盖')
    ...
    .action(async (templateName, projectName, cmd) => {
        ...
        const isExists = await isExistsFile(projectName, cmd);
        if (isExists) return;
        download(projectName, templateName);
    });
// #endregion ---------------------- init 快速创建项目 - END -------------------
